...
- **If the user names a slice**, scope is every file that slice name resolves to across the whole repo: `<slice>.spec.md`, `<slice>.design.md`, and `<slice>.plan.md` wherever each exists. Nothing wider than that slice, but check every project's `deltas/` for it, not just one. In a repo split across multiple projects, a slice's files don't have to share a directory — `design.md` lives under whichever project renders the UI, and each project can even carry its own `<slice>.spec.md` scoped to the logic it owns there (e.g. a backend's session/data contract and a frontend's own navigation-logic contract, same slice name, deliberately different content) — treat those as siblings to check together, not duplicates to collapse into one.
...
- **If the user explicitly asks for everything** ("revisa todos los specs"), honor it — but first correlate by slice name across every project's `deltas/`, so a slice with siblings in two projects is checked and reported as one slice, not two unrelated hits. Say up front that this dispatches one check per slice (not per file) and will cost proportionally, and offer to narrow first. Don't refuse, just don't default to it.
