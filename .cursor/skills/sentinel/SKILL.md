...
- **Delegate the expensive reading, don't inline it.** If confirming a claim needs broader reading/judgment (not just a grep hit), dispatch it to a subagent instead of pulling that source into the main session's context:
  - If a matching `.cursor/agents/*` subagent already exists for this kind of check (e.g. a spec-invariant-checker scoped to this slice), use it — that's exactly what it's for.
...
- Doesn't check `.cursor/docs/*` or `.cursor/rules/*` for drift — scope is a slice's spec, design, and plan only. A broader drift-checker is a different skill, not a reason to stretch this one.
