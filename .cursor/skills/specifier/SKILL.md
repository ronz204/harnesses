...
- Any `.cursor/docs/*` claim that describes this slice.
- Any `.cursor/rules/*` convention that should apply to it (e.g. a naming convention, an access-control pattern, a data-handling rule — whatever this project's actual rules cover).
...
Ask in a short, focused batch (ask directly when the choices are concrete and enumerable) rather than dumping the whole list at once — a spec or design built from five rushed answers is worse than one built from two well-considered ones, continued next turn if needed.
