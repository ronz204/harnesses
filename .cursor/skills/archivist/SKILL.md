...
- Skim what already exists in `.cursor/docs/`, `.cursor/rules/`, `.cursor/agents/`, `.cursor/skills/`, and `AGENTS.md` so you don't duplicate or contradict something that's already documented elsewhere.
...
| Product/system vision, scope boundaries, what problem the system solves, the purpose of each major part at a conceptual level | `.cursor/docs/overview.md` (or this project's equivalent) |
| Per-component functional reference: flows, data shape, what a component actually does step by step | `.cursor/docs/modules.md` (or equivalent, e.g. `components.md`) |
| System topology, stack choices + rationale, how parts communicate, cross-cutting patterns (auth, background jobs, etc.) | `.cursor/docs/structure.md` (or `architecture.md`) |
| Static infrastructure/provisioning facts: things set up once and rarely revisited — only a real category if the project has this kind of infrastructure at all | `.cursor/docs/infrastructure.md` (or equivalent) — omit the category entirely otherwise |
| A convention that must be **applied every time** a certain kind of file is written or edited (a style rule, a naming pattern, a required check) | `.cursor/rules/<topic>.mdc`, scoped via `globs`/`alwaysApply`/`description` frontmatter |
| A repeatable, isolated *task* — something you'd want to hand off with its own tool restrictions and its own context window | `.cursor/agents/<name>.md` |
| A capability Cursor should reach for across many tasks, potentially with bundled scripts/templates/references | `.cursor/skills/<name>/SKILL.md` |
...
These apply to `.cursor/docs/*.md` and `.cursor/rules/*.mdc`. They exist because earlier drafts got corrected for violating them. The volatile-vs-durable reference rule below also governs `.cursor/skills/*` and `.cursor/agents/*` — see Steps 3–4:
...
`.cursor/rules/<name>.mdc` — the `.mdc` extension is mandatory, a plain `.md` file in this directory is silently ignored. Activation is controlled by frontmatter, not a single `paths:` list:

```yaml
---
description: <short trigger description — used for Agent-Requested matching when globs is empty>
globs:
...
- Set `alwaysApply: true` (and omit `globs`) only for a rule meant to apply globally — this should be rare.
- Otherwise scope it with `globs`, matching the same priority `.claude/rules/*` gives its `paths:` frontmatter.
- Leaving `globs` empty and relying on `description` alone ("Agent Requested") is valid for a convention that's relevant contextually rather than tied to a file pattern — write the `description` as a trigger phrase in that case, the same discipline as a skill's own `description`.
- Quote every glob pattern (YAML requires it for patterns starting with `*` or `{`).
- One rule file per concern — a new convention with different trigger globs is a new rule, not an addition to an unrelated one.

---

## Step 3 — Writing subagents (`.cursor/agents/`)
...
- **Restrict `tools` to the minimum** the task needs — a review/analysis agent shouldn't get write access it doesn't use. Use `disallowedTools` instead when it's easier to name the few tools to exclude than the many to allow; omitting `tools` entirely inherits everything.
...
- Use `references/mark.template.md`: short project description, knowledge base layout, repo layout, setup/common commands, a pointer-not-copy summary of `.cursor's own permission/safety configuration, product-specific — see current Cursor docs`, and any project-wide convention that isn't already its own rule file.
