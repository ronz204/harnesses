<!--
Template for .cursor/agents/<name>.md — a subagent: a bounded, repeatable
...
tools: <Minimum tool list this task actually needs, e.g. "Read, Glob, Grep" for a review-only agent — don't grant write access it won't use. Omitting `tools` inherits everything; pair with `disallowedTools` instead when it's easier to name the few tools to exclude than the many to allow.>
...
project's actual conventions — pull specifics from .cursor/rules/ and
.cursor/docs/ rather than generic reviewer advice. Same volatile-vs-durable
