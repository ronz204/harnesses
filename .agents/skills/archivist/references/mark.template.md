...
every Antigravity session automatically. This is the one artifact in this
...
<!--
Table explaining what lives under .agents/ (and wherever specs live) and
...
| Path | Holds |
|---|---|
| `.agents/docs/` | <self-contained reference files, one concern each — e.g. architecture, module reference> |
| `.agents/rules/` | <conventions auto-loaded when a matching file is opened/edited, scoped via `paths:` frontmatter> |
| `.agents/agents/` | <bounded, repeatable subagent tasks with their own tool access> |
| `.agents/skills/` | <capabilities pulled in across tasks, e.g. this project's own archivist/specifier pair> |
...
<!-- Point at .agents/rules/ for anything conditionally-loaded; state here only what applies globally and isn't already its own rule file — e.g. a project-wide non-negotiable. Keep this short — a growing list belongs in a new rule, not here. -->
