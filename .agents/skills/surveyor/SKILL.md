...
- Check whether `.agents/docs/` already has files. Partial existing content isn't a blocker — treat it the same as Step 0 of every other skill in this family: read what's there first, only fill genuine gaps, don't duplicate or contradict it.
...
- **Infrastructure** — whether the project has static infrastructure/provisioning worth a `.agents/docs/infrastructure.md`, if Step 1 couldn't tell from config alone.
...
- Each warranted `.agents/docs/*.md` via `references/docs.template.md` — only the categories Step 1 actually found material for.
- Each rule candidate confirmed in Step 2, as its own `.agents/rules/<topic>.md` via `references/rules.template.md` — one file per concern, same as any other rule.
...
- Never creates a `.agents/rules/*.md` without the user confirming the pattern is deliberate and worth enforcing every time — unlike `AGENTS.md`/docs content, a false-positive rule actively injects wrong guidance into every future session that touches a matching file, not just a passive gap.
- Never overwrites an existing `AGENTS.md`/`.agents/docs/*` that already has real content without the user confirming a deliberate refresh — see Step 0.
